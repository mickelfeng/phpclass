...
brew install pkg_name #安装包 想了解更多地信息，请参看homebrew
安装nginx

	brew search nginx
	brew install nginx 当前的最新版本是1.4.4。
配置
	cd /usr/local/etc/nginx/
	mkdir conf.d
	vim nginx.conf
	vim ./conf.d/default.conf nginx.conf内容,
	worker_processes  1; 
...
Mac OSX 10.9的系统自带了PHP、php-fpm，省去了安装php-fpm的麻烦。 这里需要简单地修改下php-fpm的配置，否则运行php-fpm会报错。
	...
	vim /private/etc/php-fpm.conf 修改php-fpm.conf文件中的error_log项，默认该项被注释掉，
	这里需要去注释并且修改为error_log = /usr/local/var/log/php-fpm.log。如果不修改该值，运行php-fpm的时候会提示log文件输出路径不存在的错误。
安装mysql
安装
	brew install mysql 常用命令
	mysql.server start #启动mysql服务
	mysql.server stop #关闭mysql服务 配置 
...
在之前nginx配置文件default.conf中设置的root项对应的文件夹下创建测试文件index.php:
	...
	修改配置文件，重启nginx服务，sudo nginx -s reload 启动php服务，sudo php-fpm； 
